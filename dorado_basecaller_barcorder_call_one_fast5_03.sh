...
# tested with dorado 0.3.4 and ONT barcoding software version 7.1.4+d7df870c0 (Mk1c distribution)
...
doradobin=/applications/ont_dorado_versions/dorado-0.3.4-linux-arm64/bin/dorado
barcoderbin=/opt/ont/dorado/bin/ont_barcoder
bcmodelpath=/applications/ont_dorado_versions/dorado-0.3.4-models
...
for f5 in `find $inputdir -name '*.fast5'`; do
	...
	mkdir -p $bcd
	cd $bcd
	allq=$bcd/fastq_all.fastq
	if [ -f $allq ]; then
  	echo "File "$allq" exists, skipping basecalling."
	else
  	echo "Generating "$allq" by basecalling."
		$doradobin basecaller --device cuda:all --batchsize 64 --emit-fastq $bcmodelpath/$bcmodel $d5 > $allq
		calledone="yes"
	fi
	cd $d5
	bard=$d5/barcoding
	if [ -f $d5/$b5.fastq ]; then
		...
		largest=0
		largestfq=""
		for fq in `find $bard/barcode* -name *.fastq`; do
			si=`stat --printf="%s" $fq`
			echo "----- "$fq" = "$si" bytes"
