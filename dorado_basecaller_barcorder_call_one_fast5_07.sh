...
# BCMODEL=dna_r9.4.1_e8_hac@v3.3 / dna_r10.4.1_e8.2_400bps_hac@v4.3.0 / dna_r10.4.1_e8.2_260bps_hac@v4.1.0
...
# data pathes
datadir=/var/lib/minknow/data
inputdir=/var/lib/minknow/data/$samplename
...
bcmodelpath=/applications/ont_dorado_versions/dorado-0.5.0-models
...
for f5 in `find $inputdir -name '*.fast5'`; do
	echo "processing "$f5":"
	b5=`echo $(basename -s .fast5 ${f5}) | sed -e s/barcode/bacd/g | sed -e s/unclassified/uncl/g ` # replace "barcode" keyword to avoid confusion, mainly applies to pre-basecalled files with barcode indicators in their names.
	echo "basename: "$b5
	...
	if [[ $largestbc != "" ]]; then
		...
		outputmk5d=`echo $datadir"/"$samplename"_ND/"$samplename"_ND/"$bcmodel-$barkit"/fast5_pass/"$largestbc`
		outputmkqd=`echo $datadir"/"$samplename"_ND/"$samplename"_ND/"$bcmodel-$barkit"/fastq_pass/"$largestbc`
		mkdir -p $outputmk5d
		mkdir -p $outputmkqd
