...
versionString="47"                                   # version string of this application
...
gpuid=0                                             # CUDA device ID for cupy operations, e.g., StDev
f5cCudaDevId="0"                                    # CUDA device ID for f5c
...
#readsPerFile="40"                                   # number of reads per file. 400 works well on the Jetson AGX. Higher numbers increase batch size and RAM usage, lower numbers use more I/O resouces due to more frequent reloading of alignment reference
...
analysisExclude=["_foobar_"] # testing ["_TestRun_"]                       # string patterns in sample names that exclude data from downstream analysis, e.g., test runs
...
cherrypyThreads=500                                 # number of concurrent threads allowed to CherryPy, decrease in case of performance problems, default = 100
...
cnvLinkPrefix="http://meqneuropat02.uhbs.ch:3838/umap_coords/www/" # "http://s1665.rootserver.io/umapplot01/" # URL prefix to load PDF with CNV plot for a given Sentrix ID
cnvLinkSuffix="_CNV_IFPBasel_annotations.pdf"       # URL prefix to load PDF with CNV plot 
epidipUmapCoordUrlRoot="http://meqneuropat02.uhbs.ch:3838/umap_coords/IFP_all/" # "http://s1665.rootserver.io/umap_links/" # URL profix to load current EpiDiP coordinates
epidipUmapCoordUrlFiles=["UMAP_all_bVals_top_25000.xlsx", # list of UMAP coordinate files hosted on EpiDiP
...
#f5cBin="/applications/f5c/f5c"                      
f5cBin="export HDF5_PLUGIN_PATH=/home/minit/.local/hdf5/lib/plugin; /applications/f5c_r10/f5c-v1.2/f5c" # f5c binary launch command
#minimap2Bin="/applications/nanopolish/minimap2/minimap2" # minimap2 binary location (absolute path)
minimap2Bin="/opt/ont/dorado/bin/minimap2-2.24"     # minimap2 binary location (absolute path)
...
basecallerBin="/applications/ontGuppyVersions/ont-guppy_6.5.7/bin/guppy_basecaller"
basecallerOptions='-c dna_r10.4.1_e8.2_400bps_sup.cfg --disable_qscore_filtering --compress_fastq --device cuda:0,1,2 --num_callers 4 --gpu_runners_per_device 1 --disable_pings'
slow5toolsBin="export HDF5_PLUGIN_PATH=/home/minit/.local/hdf5/lib/plugin; /applications/slow5tools/slow5tools/slow5tools" # needs to import the location of the VBZ/HDF5 plugins 
basecallWrapperScript="/applications/nanodip/dorado_basecaller_barcorder_call_one_fast5_04.sh"
bcmodelR9="basecallModel=dna_r9.4.1_e8_hac@v3.3&barcodeSet=SQK-RBK004"
bcmodelR10f="basecallModel=dna_r10.4.1_e8.2_400bps_hac@v4.2.0&barcodeSet=SQK-RBK114-24"
bcmodelR10s="basecallModel=dna_r10.4.1_e8.2_260bps_hac@v4.1.0&barcodeSet=SQK-RBK114-24"
...
# config
#nanodipAppDir="/applications/nanodip"
#nanodipRefDir="/applications/reference_data"        # NanoDiP reference data directory
#minknowDataDir="/var/lib/minknow/data"
#nanodipOutputDir="/media/minknow/leer_satassd_tes/f5c_output"   # location to write intermediate analysis data, i.e. methylation and alignment files
#refgenomemmi=nanodipRefDir+"/minimap_data/hg19_nanodip.mmi" # human reference genome minimap2 mmi
#refgenomefa=nanodipRefDir+"/minimap_data/hg19.fa"   # human reference genome
#f5cBin="export HDF5_PLUGIN_PATH=/home/minknow/.local/hdf5/lib/plugin; /applications/f5c_r10/f5c-v1.2/f5c"
#f5cCudaDevId="1"
#minimap2Bin="/opt/ont/dorado/bin/minimap2-2.24" # minimap2 binary location (absolute path)
#basecallerBin="/applications/ontGuppyVersions/ont-guppy_6.5.7/bin/guppy_basecaller"
#basecallerOptions='-c dna_r10.4.1_e8.2_400bps_sup.cfg --disable_qscore_filtering --compress_fastq --device cuda:0,1,2 --num_callers 4 --gpu_runners_per_device 1 --disable_pings'
#basecallerBin="/applications/ontGuppyVersions/ont-guppy_6.5.7/bin/guppy_basecall_client"
#basecallerOptions='-c dna_r10.4.1_e8.2_400bps_sup.cfg --disable_qscore_filtering --compress_fastq --disable_pings --port /tmp/.guppy/5556'
#samtoolsBin="/applications/samtools/samtools"       # samtools binary location (absolute path)
#slow5toolsBin="export HDF5_PLUGIN_PATH=/home/minknow/.local/hdf5/lib/plugin; /applications/slow5tools/slow5tools/slow5tools" # needs to import the location of the VBZ/HDF5 plugins 
    ...
    for b in range(len(barcodeNames)):
        ...
        for f in fast5List:
            if barcodeNames[b] in f:
                fsize=os.path.getsize(f) # determine fast5 file size
                ...
                if os.path.exists(thisAnalysisDir+"/"+thisBcFileName+"-methoverlapcount.txt")==False:
                    ...
                    if analyzedCount==0:
                        ...
                        if os.path.exists(bamSortedPath)==False and os.path.exists(fastqPath+".index")==False and os.path.exists(fastqPath+"index.fai")==False and os.path.exists(fastqPath+"index.gzi")==False:
                            cmd=minimap2Bin+" -a -x map-ont "+refgenomemmi+" "+fastqPath+" -t 12 | "+samtoolsBin+" sort -T tmp | "+samtoolsBin+" view -bSq 20 >"+bamSortedPath
                            logpr(verbosity,cmd)
                        ...
                        if os.path.exists(freqTsvPath)==False: # f5c meth-freq -i [meth.tsv] > [freq.tsv]
                            cmd=f5cBin+" meth-freq -i "+methTsvPath+" > "+freqTsvPath
                            logpr(verbosity,cmd)
                            p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
                            p.wait() # wait until process is completed
                        else:
                    ...
                    completedCount+=1
    return "Target = "+targetBcSet+"<br>Methylation called for "+str(calledList)+". "+str(completedCount+analyzedCount)+"/"+str(len(thisBcFast5))  
...
def thisRunWatcherTerminator(deviceString,sampleName): # run terminator site script; periodically reloads and terminates run upon reaching target base count
...
def generateEpidipReport(sentrixid, referenceFile, umapFile): # generate PDF report from EpiDiP data (based on case Sentrix ID)
    downloadLog=downloadEpidipCoordinates(umapFile,sentrixid,referenceFile)
    try:
...
def cnvPlotPage(mySentrix,searchGenes):
    ...
    if mySentrix and searchGenes:
        try:
            ...
            h=h+"<br><font color='red'>Data could not be plotted.</font>"
...
def analysisLaunchTable(): # present a table from which analyses can be started in a post-hoc manner
    ...
    lt=lt+"<thead><tr><th align='left'><b>Sample ID</b></th><th align='left'><b>R9</b></th><th align='left'><b>R10f</b></th><th align='left'><b>R10s</b></th><th align='left'><b>CpGs</b></th><th align='left'><b>CpGs+<i>U</i></b></th><th align='left'><b>CNV</b></th>"
    ...
    for r in range(len(allowedRunFolders)):
        ...
        if allowedRunFolders[r].endswith("_ND"): # has already been basecalled and barcoded by NanoDiP
            lt=lt+"<td></td><td></td><td></td>"
        else:
            lt=lt+"<td><a href='./analysisLauncher?functionName=basecallPoller&sampleName="+allowedRunFolders[r]+"&"+bcmodelR9+"' target='_blank' rel='noopener noreferrer' title='"+allowedRunFolders[r]+": basecall R9'>R9</a></td>"
            lt=lt+"<td><a href='./analysisLauncher?functionName=basecallPoller&sampleName="+allowedRunFolders[r]+"&"+bcmodelR10f+"' target='_blank' rel='noopener noreferrer' title='"+allowedRunFolders[r]+": basecall R10 400bps'>R10f</a></td>"
            lt=lt+"<td><a href='./analysisLauncher?functionName=basecallPoller&sampleName="+allowedRunFolders[r]+"&"+bcmodelR10s+"' target='_blank' rel='noopener noreferrer' title='"+allowedRunFolders[r]+": basecall R10 260bps'>R10s</a></td>"
        ...
        lt=lt+"<td><a href='./analysisLauncher?functionName=methylationPoller&sampleName="+allowedRunFolders[r]+"&refAnno=includeunclassified' target='_blank' rel='noopener noreferrer' title='"+allowedRunFolders[r]+": all CpGs including unclassified reads'>get CpGs+U</a></td>"
...
def menuheader(n,r): # generate a universal website header for the UI pages that contains a simple main menu
    menuList=[["index","Overview","General system information"],
              ...
              ["bisdipReport","bisDiP report","Generate report PDFs for bisDiP cases"],
...
class MinKnowIfPApi(object): # the CherryPy Web UI class that defines entrypoints and function calls
    ...
    @cherrypy.expose
    def bisdipReport(self, sentrixid=None, refAnno=None):
        myString=menuheader(11,0)
        myString=myString+"bisDiP report, under construction"
        if sentrixid and refAnno:
            myString=myString+"bisdipReport report for "+sentrixid+" with "+refAnno+", under construction"
        return myString
    
    @cherrypy.expose
    def about(self):
        myString=menuheader(12,0)+'''
